...
class pppCompiler(ast.NodeTransformer):
    def __init__(self):
        ...
        self.ifctr = 1          #number of if statements
        self.elsectr = 1        #number of else statements
        self.whilectr = 1       #number of while statements
        self.fnctr = 1
    ...
    def visit_While(self, node):
        ...
        if hasattr(node.test, 'left'):
            ...
            right, op = None, ''
        if right is None:
            self.codestr += ["LDWR {0}\nJMP{1}Z end_while_label_{2}".format(left,op,currentWhileCtr)]
        else:
    ...
    def preprocessCode(self, code):
        ...
        preprocessed_code = re.sub(r"\s*const\s*(\S+)\s*=\s*(\S+)", self.collectConstants, code)
        ...
        preprocessed_code = re.sub(r"\s*parameter\s*<(\S+)>\s*(\S+)\s*=\s*(\S+)\s*(\S+)", self.collectTypedParameters, preprocessed_code)
        preprocessed_code = re.sub(r"\s*parameter\s*<(\S+)>\s*(\S+)\s*=\s*(\S+)", self.collectTypedParameters, preprocessed_code)
        preprocessed_code = re.sub(r"\s*parameter\s*(\S+)\s*=\s*(\S+)\s*(\S+)", self.collectParameters, preprocessed_code)
        preprocessed_code = re.sub(r"\s*parameter\s*(\S+)\s*=\s*(\S+)", self.collectParameters, preprocessed_code)
        ...
        preprocessed_code = re.sub(r"\s*counters\s*(\S+)", self.collectCounters, preprocessed_code)
...
class ppVirtualMachine:
    ...
    def findLabelLocations(self, code):
        ...
        for n, line in enumerate(self.code):
            m = re.match("(end_if_label_\d+):", line)
            if m:
                if m.group(1) in self.labelDict.keys():
                    raise Exception("Label {} already in dict!".format(m.group(1)))
                self.labelDict[m.group(1)] = n
            m = re.match("(while_label_\d+):", line)
            if m:
                if m.group(1) in self.labelDict.keys():
                    raise Exception("Label {} already in dict!".format(m.group(1)))
                self.labelDict[m.group(1)] = n
            m = re.match("(end_while_label_\d+):", line)
            if m:
                if m.group(1) in self.labelDict.keys():
                    raise Exception("Label {} already in dict!".format(m.group(1)))
                self.labelDict[m.group(1)] = n
            m = re.match("(else_label_\d+):", line)
            if m:
                if m.group(1) in self.labelDict.keys():
                    raise Exception("Label {} already in dict!".format(m.group(1)))
                self.labelDict[m.group(1)] = n
            m = re.match("(end_function_label_\d+):", line)
            if m:
                if m.group(1) in self.labelDict.keys():
                    raise Exception("Label {} already in dict!".format(m.group(1)))
                self.labelDict[m.group(1)] = n
...
def evalRawCode(code):
    ...
    d = dict()
    ex = exec(compile(preprocessed_code, '<string>', 'exec', optimize=2),d)
    return d
...
def printAllSettings(code):
    if code.group(2)[0] != '#':
        return "{0}{1} = {2}{0}print({1})".format(code.group(1),code.group(2),code.group(3))
